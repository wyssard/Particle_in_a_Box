...
class Anti_Symmetric_Boundary(Boundary):
    def __init__(self, L: float, gamma: float, l_to_kl_mapper_ref: l_to_kl_mapper) -> None:
        self._x_space_projector = X_Space_Projector(L, gamma, l_to_kl_mapper_ref)
        self._k_space_projector = K_Space_Projector(L, gamma, l_to_kl_mapper_ref)
        self._bra_l1_x_ket_l2 = Bra_l1_x_Ket_l2(L, gamma, l_to_kl_mapper_ref)
        self._bra_l1_pR_ket_l2 = Bra_l1_pR_Ket_l2(L, gamma, l_to_kl_mapper_ref)
        self._gamma_to_k = Gamma_to_k(L, gamma)

    @property
    def X_Space_Projector(self) -> Energy_State_Projector:
        return self._x_space_projector

    @property
    def K_Space_Projector(self) -> Energy_State_Projector:
        return self._k_space_projector

    @property
    def Bra_l1_x_Ket_l2(self) -> Energy_State_Matrix_Elements:
        return self._bra_l1_x_ket_l2

    @property
    def Bra_l1_pR_Ket_l2(self) -> Energy_State_Matrix_Elements:
        return self._bra_l1_pR_ket_l2

    @property
    def Gamma_to_k(self) -> Gamma_to_k_Base:
        return self._gamma_to_k